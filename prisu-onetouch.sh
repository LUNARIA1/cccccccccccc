...
DOWNLOAD_URL="https://github.com/PocketRisu/PocketRisu/releases/download/${VERSION}/${TARBALL}"

log "포터블 패키지 다운로드 중... ($DOWNLOAD_URL)"
curl -fsSL "$DOWNLOAD_URL" -o /tmp/pocketrisu.tar.gz \
  || err "다운로드 실패. 버전 또는 URL을 확인하세요: $DOWNLOAD_URL"

log "압축 해제 중..."
mkdir -p "$INSTALL_DIR"
tar -xzf /tmp/pocketrisu.tar.gz -C "$INSTALL_DIR" --strip-components=1
rm /tmp/pocketrisu.tar.gz
