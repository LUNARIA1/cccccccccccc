...
# HuggingFace 토큰 (이 모델은 불필요 — 게이트 모델일 때만 사용)
HF_TOKEN="${HF_TOKEN:-}"
...
pkg install -y \
    git cmake make clang curl golang \
    libicu libxml2 \
    2>/dev/null
...
if [ ! -f "$MODEL_PATH" ]; then
    ...
    if [ -z "$DL_SIZE" ] || [ "$DL_SIZE" -lt 100 ]; then
        ...
        # 인증 실패면 토큰 입력받기
        if echo "$DL_CONTENT" | grep -qi "invalid\|unauthorized\|login\|401"; then
            warn "인증 필요. HuggingFace 토큰을 입력하세요"
            warn "발급: https://huggingface.co/settings/tokens"
            echo ""
            printf "  HF Token (hf_...): "
            read -r HF_TOKEN
            echo ""
            if [ -n "$HF_TOKEN" ]; then
                ...
                if [ -z "$DL_SIZE" ] || [ "$DL_SIZE" -lt 100 ]; then
                    rm -f "$MODEL_PATH"
                    fail "토큰 인증 후에도 다운로드 실패"
                fi
            else
                fail "토큰이 입력되지 않음"
            fi
        else
            fail "다운로드 실패. 응답: $DL_CONTENT"
        fi
    fi
...
CF_BIN="$HOME/go/bin/cloudflared"

if [ -x "$CF_BIN" ]; then
    info "cloudflared 이미 설치됨"
else
    CF_SRC="$WORK/cloudflared-src"
    info "cloudflared 빌드 중... (Go 컴파일, 2~3분 소요)"
    if [ ! -d "$CF_SRC/.git" ]; then
        rm -rf "$CF_SRC"
        git clone --depth 1 https://github.com/cloudflare/cloudflared "$CF_SRC"
    fi
    mkdir -p "$HOME/go/bin"
    cd "$CF_SRC"
    go build -o "$CF_BIN" ./cmd/cloudflared 2>&1 | tail -5
    cd "$HOME"
    [ -x "$CF_BIN" ] || fail "cloudflared 빌드 실패"
    rm -rf "$CF_SRC"
