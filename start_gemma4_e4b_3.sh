...
if [ -x "$SERVER_BIN" ]; then
    ...
    # ── 방법 1: GitHub releases 페이지에서 직접 URL 스크래핑 (API 불필요) ──
    info "GitHub에서 최신 Android ARM64 릴리즈 확인 중..."
    ASSET_URL=$(curl -sL "https://github.com/ggml-org/llama.cpp/releases/latest" \
        | grep -oE '/ggml-org/llama\.cpp/releases/download/[^"]*android[^"]*arm64[^"]*\.zip' \
        | head -1) || true
    ...
    # ── 방법 2: GitHub API + jq ──
    if [ -z "$ASSET_URL" ]; then
        warn "방법 1 실패, GitHub API 시도 중..."
        ASSET_URL=$(curl -sL "https://api.github.com/repos/ggml-org/llama.cpp/releases/latest" \
            | jq -r '.assets[] | select(.name | test("android.*arm64.*\\.zip$")) | .browser_download_url' \
            2>/dev/null | head -1) || true
        [ "$ASSET_URL" = "null" ] && ASSET_URL=""
        [ -n "$ASSET_URL" ] && info "찾음 (GitHub API)"
    fi

    # ── 방법 3: 태그 감지 후 URL 직접 조합 ──
    if [ -z "$ASSET_URL" ]; then
        warn "방법 2 실패, 태그 감지 시도 중..."
        LATEST_TAG=$(curl -sI "https://github.com/ggml-org/llama.cpp/releases/latest" \
            | grep -i '^location:' | grep -oE 'b[0-9]+' | tail -1) || true
        if [ -n "$LATEST_TAG" ]; then
            ASSET_URL="https://github.com/ggml-org/llama.cpp/releases/download/${LATEST_TAG}/llama-${LATEST_TAG}-bin-android-arm64.zip"
            info "찾음 (태그: $LATEST_TAG)"
        fi
    fi
    ...
    if [ -z "$ASSET_URL" ]; then
        echo ""
        warn "자동 감지 실패. 수동으로 URL을 입력해 주세요."
        warn "여기서 android-arm64 zip 링크를 복사하세요:"
        warn "  https://github.com/ggml-org/llama.cpp/releases/latest"
        echo ""
        printf "  ZIP URL: "
        read -r ASSET_URL
        [ -z "$ASSET_URL" ] && fail "URL이 입력되지 않았습니다."
    fi
    ...
    # llama-server 바이너리 찾기 (폴더 구조가 릴리즈마다 다를 수 있음)
